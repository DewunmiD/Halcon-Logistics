...
>
  Signin
      ...
      >
        ...
        <IoMdMenu className='text-white w-[40px] h-[40px]'/>
      </button>

      {/* Mobile menu */}
      
      <div
  className={`fixed top-0 left-0 h-full w-full bg-gray-800 opacity-75 transition-all duration-300 ease-in-out ${
    isOpen ? 'translate-x-0' : '-translate-x-full'
  }`}
>
        <ul className="flex flex-col space-y-4 px-4 py-8 text-center font-bold text-red-600">
          <li>
            <Link to="/home" className="hover:text-gray-400">
              Home
            </Link>
          ...
          <li>
            <button className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700" onClick={() => navigate('/signin')}>
              Sign In
            </button>
