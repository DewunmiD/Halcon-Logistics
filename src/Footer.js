...
function Footer() {
  return (
    <div className='mt-[200px] md:mt-[100px] lg:mt-[50px]w-full !px-0 !mx-0 !overflow-hidden hidden md:block'>
      <div className="flex justify-center items-center gap-4 gap-x-8 md:flex-wrap lg:flex-nowrap">
      <div className='rounded-full bg-red-600 w-[30px] h-[30px] ml-[5px] mt-[-100px] md:w-[20px] md:h-[20px] lg:w-[30px] lg:h-[30px]'></div>
      <div className='bg-gray-400 text-left w-[450px] h-[250px] ml-[50px] rounded-2xl md:w-[300px] md:h-[200px] lg:w-[450px] lg:h-[250px]'>
        ...
        <div className="container1 flex justify-start gap-8 text-2xl ml-[20px] mt-[10px]">
          <FiLinkedin onClick={() => window.open('(link unavailable)', '_blank')} />
          <FaTwitter onClick={() => window.open('(link unavailable)', '_blank')} />
          <FaInstagram onClick={() => window.open('(link unavailable)', '_blank')} />
          <FaFacebookF onClick={() => window.open('(link unavailable)', '_blank')} />
        </div>
      </div>

      <div className='bg-gray-400 text-left w-[450px] h-[250px] rounded-2xl md:w-[300px] md:h-[200px] lg:w-[450px] lg:h-[250px]'>
      ...
      <div className='rounded-full aspect-ratio-1 bg-red-600 w-[30px] h-[30px] mr-[5px] mb-[-100px] md:w-[20px] md:h-[20px] lg:w-[30px] lg:h-[30px]'></div>
      </div>

     


      <div className="flex justify-center items-center gap-1 md:flex-wrap lg:flex-nowrap">
  <div className="w-1/2 flex-shrink-0 "><img src={images} alt=''/>

    <div className='flex gap-5 ml-[20px] font-semibold'>
      <button 
        className='flex justify-start items-center gap-2 bg-red-200 mr-2 w-[150px] h-[70px] rounded-lg text-red-600 md:w-[100px] md:h-[50px] lg:w-[150px] lg:h-[70px]'
        onClick={() => window.open('(link unavailable)<app-name>/id<app-id>', '_blank')}
      >
        <FaApple className='text-xl text-red-600 ml-[20px]'/>Get it on<br/>App Store
      </button>

      <button 
        className='flex justify-start items-center gap-2 bg-red-200 w-[150px] h-[70px]'
